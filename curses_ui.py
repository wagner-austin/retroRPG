...
# version: 2.2
# Summary: A curses-based front-end implementing IGameRenderer & IGameInput for user interaction.
...
from interfaces import IGameRenderer, IGameInput
...
class CursesGameInput(IGameInput):
    """
    Implements IGameInput for curses:
      - get_actions() reads the keyboard buffer
...
class CursesGameRenderer(IGameRenderer):
    """
    Implements IGameRenderer using curses:
      - render(model) checks model.full_redraw_needed => full redraw or partial update
      - on_camera_move(dx, dy, model) => partial scroll if possible
      - get_visible_size() => (max_w, max_h - top_offset)
      - prompt_yes_no(question) => curses-based yes/no
      - get_curses_window() => returns self.stdscr for external UI calls
    """
    ...
    def get_curses_window(self):
        """
        Return the underlying curses window (stdscr) for use by external UI functions
        like save_map_ui().
        """
        return self.stdscr
